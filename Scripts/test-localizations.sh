...
LOCALIZATION_APP="$BUILD_DIR/tests/$TEST_ARCH/HueLocalizationTests.app"
...
plutil -replace CFBundleExecutable -string HueLocalizationTests "$LOCALIZATION_APP/Contents/Info.plist"
plutil -replace CFBundleIdentifier -string local.hue.localization-tests "$LOCALIZATION_APP/Contents/Info.plist"
...
xcrun swiftc -swift-version 5 -target "$TEST_ARCH-apple-macos$MINIMUM_MACOS" \
    ...
    -o "$LOCALIZATION_APP/Contents/MacOS/HueLocalizationTests"
